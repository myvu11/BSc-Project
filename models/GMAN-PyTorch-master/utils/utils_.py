...
# Masked mape code from model Graph WaveNet util.py
...
def metric(pred, label):
    ...
    mape = masked_mape(pred, label, 0.0)
...
def load_data(args):
    ...
    df = df.set_index(df.index.values).asfreq("5min") # FIX FOR # pd.DatetimeIndex(df.index) && time.freq.delta.total_seconds() NOT WORKING 
    time = df.index # FIX FOR # pd.DatetimeIndex(df.index) && time.freq.delta.total_seconds() NOT WORKING 
